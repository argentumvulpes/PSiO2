...
int main()
{
    ...
    std::vector<Game> pl;
    for(int i=0; i<14; i++){
    Platform platf(sf::Vector2f(rand()%550,i*(-80)+1000), platform);
    ...
    int points = 0;
    bool go =false;
    std::cout<<"Do you want to Quit?? press Q";
    ...
    while (window.isOpen()) {
        ...
        else if(high_score < -50){go = true;}
        else if(high_score > 2000){go = true;}
